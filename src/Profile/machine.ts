...
export const lookupPlaces = async (searchTerm: string) => {
  const response = await fetch(
    `${CONSTANTS.MAPBOX_PLACES_API}/${searchTerm}.json?access_token=${CONSTANTS.MAPBOX_ACCESS_TOKEN}`,
    {
      method: 'GET',
    },
  );
...
): Promise<{ location: Location; targetState: string }> => {
  ...
  createMachine<UserContext, UserEvents>(
    {
      ...
      states: {
        ...
        findLocation: {
          invoke: {
            src: (context, event) =>
              findLocation(event as ENABLE_LOCATION_EVENT),
            onDone: [
              {
                target: 'edit.valid',
                actions: ['setFoundLocation'],
                cond: (context, event) =>
                  event.data.targetState.includes('display'),
              },
              {
                target: 'create.valid',
                actions: ['setFoundLocation'],
                cond: (context, event) =>
                  event.data.targetState.includes('create') &&
                  event.data.targetState.includes('valid'),
              },
              {
                ...
                cond: (context, event) =>
                  event.data.targetState.includes('create') &&
                  event.data.targetState.includes('invalid'),
              },
              {
                target: 'edit.valid',
                actions: ['setFoundLocation'],
                cond: (context, event) =>
                  event.data.targetState.includes('edit') &&
                  event.data.targetState.includes('valid'),
              },
              {
                target: 'edit.invalid',
    ...
    },
    {
      actions: {
        setWallet: assign({
          walletId: (context, event) =>
            (event as CONNECT_EVENT).wallet.publicKey?.toBase58() || '',
          signMessage: (context, event) =>
            (event as CONNECT_EVENT).wallet.signMessage,
          signTransaction: (context, event) =>
            (event as CONNECT_EVENT).wallet.signTransaction,
          sendTransaction: (context, event) =>
            (event as CONNECT_EVENT).wallet.sendTransaction,
        ...
        setUser: assign({
          ...
          location: (context, event: any) => ({
            ...
            coordinates: [
              parseFloat(event.data.location.latitude),
              parseFloat(event.data.location.longitude),
            ],
          }),
        ...
        setNickName: assign({
          nickName: (context, event) =>
            (event as INPUT_NICK_NAME_EVENT).nickName,
        }),
        setTwitter: assign({
        ...
        setTelegram: assign({
          telegram: (context, event) =>
            (event as INPUT_TELEGRAM_EVENT).telegram,
        }),
        setDiscord: assign({
        ...
        setIsHardware: assign({
          isHardware: (context, event: any) =>
            (event as IS_HARDWARE_WALLET_EVENT).isHardware,
        }),
